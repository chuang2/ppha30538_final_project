...
```{python}
#Issue with any plot is that bulk of entries are nonresponses so sample is skewed depending on who responded.
```

```{python}
#Frequency of volunteer from a given state
#frequency of volunteers by hours and location? rural vs urban areas

#if focusing on polarization, better to focus on where volunteers are coming from 

#use the shapefile from ps5
...
data = pd.DataFrame({
    'state': ['California', 'Texas', 'New York', 'California', 'Texas', 'Florida', 'California']
})
...
us_map = world[world['continent'] == 'North America'].loc[world['name'] == 'United States', 'geometry'].values[0]

# You can load US states geometry if you prefer a more detailed map
states = gpd.read_file(gpd.datasets.get_path('naturalearth_lowres')).loc[world['name'] == 'United States']

# Merge the counts with the GeoDataFrame of US states
# Assuming your 'state_counts' dataframe has state names
merged = states.set_index('name').join(state_counts.set_index('state'))

# Plot the map with a color scale based on the 'count' column
fig, ax = plt.subplots(1, 1, figsize=(15, 10))

# Plot states with a color map based on state count
merged.plot(column='count', ax=ax, legend=True,
            legend_kwds={'label': "Number of Times a State Appears",
                         'orientation': "horizontal"},
            cmap='Blues')

# Add title
ax.set_title("Number of Times a State Appears in Dataset")
