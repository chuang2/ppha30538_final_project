...
chart.show()
chart.save('chart1.png', format='png')
```
...
chart.show()
chart.save('chart2.png', format='png')
```
...
chart.show()
chart.save('chart3.png', format='png')
```
...
# Filter out invalid or unnecessary rows
...
# Filter the dataset for those who have voted or not voted
if_voted = volunteering_frequencies.loc[volunteering_frequencies['Voted_In_Local_Election'].isin(['Yes', 'No'])]

# Further filter for valid news consumption frequencies
...
# Mapping news consumption frequency to colors
...
chart = alt.Chart(counts).mark_point(filled=True).encode(
    y=alt.Y('Frequency_Of_News_Consumption:N', title='News Consumption Frequency'),  # Y-axis: News Consumption Frequency
    x=alt.X('Frequency:Q', title='Count'),  # X-axis: Frequency (Count)
    shape=alt.Shape('Voted_In_Local_Election:N',  # Shape indicates whether they voted
                    title='Voted in Local Election',
                    scale=alt.Scale(domain=['Yes', 'No'], range=['circle', 'square'])),  # Circle for Yes, square for No
    color=alt.Color('Volunteering_Frequency:N',  # Color by Volunteering Frequency
                    ...
                    scale=alt.Scale(domain=['Not at All', 'A Few Times a Month', 'A Few Times a Week', 'Once a Month', 'Less Than Once a Month'],
                                    range=['blue', 'green', 'orange', 'red', 'purple'])),  # Colors for each Volunteering Frequency
    size=alt.SizeValue(100),
    tooltip=['Volunteering_Frequency', 'Frequency', 'Voted_In_Local_Election', 'Frequency_Of_News_Consumption']  # Tooltip with relevant details
).properties(
...
# Optionally, save the chart as a PNG
chart.save('news_volunteering_voting_chart.png', format='png')
```
...
chart = alt.Chart(counts).mark_bar().encode(
    y=alt.Y('Volunteering_Frequency:N', title='Volunteering Frequency'),  # Y-axis: Volunteering Frequency
    x=alt.X('Frequency:Q', title='Count'),  # X-axis: Frequency (Count)
    color=alt.Color('Voted_In_Local_Election:N',  # Color indicates if the person voted
                    title='Voted in Local Election',
                    scale=alt.Scale(domain=['Yes', 'No'], range=['blue', 'orange'])),  # Blue for Yes, Orange for No
    tooltip=['Volunteering_Frequency', 'Frequency', 'Voted_In_Local_Election']  # Tooltip with details
).properties(
    title='Volunteering Frequency vs. Voting Behavior (News Consumption: Basically Every Day)',
    width=400,
    height=400
)
...
# Relationship between news consumption and hours volunteered

```{python}
summary_stats = filtered_data['Hours_Spent_Volunteering'].describe()
print(summary_stats)

print('min', filtered_data['Hours_Spent_Volunteering'].max())
print('max', filtered_data['Hours_Spent_Volunteering'].min())


# Median value
median_hours = filtered_data['Hours_Spent_Volunteering'].median()
print(f"Median hours spent volunteering: {median_hours}")

# Mode value
mode_hours = filtered_data['Hours_Spent_Volunteering'].mode()[0]  # mode returns a series, so take the first value
print(f"Most common hours spent volunteering: {mode_hours}")

# Variance
variance_hours = filtered_data['Hours_Spent_Volunteering'].var()
print(f"Variance of hours spent volunteering: {variance_hours}")
...
# Step 1: Filter the data to exclude unwanted entries
...
# Step 2: Create a box plot of hours spent volunteering by news consumption frequency
chart = alt.Chart(filtered_data).mark_boxplot(extent='min-max').encode(
    x=alt.X('Frequency_Of_News_Consumption:N', title='News Consumption Frequency'),  # X-axis: News consumption frequency
    y=alt.Y('Hours_Spent_Volunteering:Q', 
            title='Hours Spent Volunteering', 
            scale=alt.Scale(domain=[0, 200])),  # Y-axis with range limit at 200
    tooltip=['Frequency_Of_News_Consumption', 'Hours_Spent_Volunteering']  # Tooltip with details
).properties(
...
print(average_hours_per_states)
```


# graphing the above but only labeling top and bottom 5 on averages
```{python}

scatter_plot = alt.Chart(average_hours_per_state).mark_point(size=100, color='blue').encode(
    x=alt.X('US State:N', title='US State', sort='-y'),  # X-axis: US States
    y=alt.Y('Hours_Spent_Volunteering:Q', title='Average Hours Spent Volunteering')  # Y-axis: Average Hours
)

# Step 2: Select top 5 and bottom 5 states for labeling
top_5_states = average_hours_per_state.nlargest(5, 'Hours_Spent_Volunteering')
bottom_5_states = average_hours_per_state.nsmallest(5, 'Hours_Spent_Volunteering')

# Step 3: Create text labels for the top and bottom 5 states
top_bottom_labels = alt.Chart(pd.concat([top_5_states, bottom_5_states])).mark_text(
    align='left',
    baseline='middle',
    dx=15,  # Slightly offset the text to the right of the point
    fontSize=14
).encode(
    x=alt.X('US State:N', title=' ', sort='-y'),  # X-axis: US States
    y=alt.Y('Hours_Spent_Volunteering:Q'),  # Y-axis: Average Hours
    text='US State:N'  # Add state labels
)

# Step 4: Combine scatter plot and labels
chart = scatter_plot + top_bottom_labels

# Step 5: Show the chart
chart.properties(
    title='Top and Bottom 5 States by Average Hours Spent Volunteering'
).configure_axisX(
    labelAngle=-90  # Rotate X-axis labels for better readability
)

chart.show()
```

# now only showing top and bottom 10 states per hours volunteered
```{python}
top_10_states = average_hours_per_state.nlargest(5, 'Hours_Spent_Volunteering')
bottom_10_states = average_hours_per_state.nsmallest(5, 'Hours_Spent_Volunteering')

# Combine top and bottom 10 into one DataFrame
top_bottom_states = pd.concat([top_10_states, bottom_10_states])

# Step 2: Create the scatter plot for the top and bottom 10 states
scatter_plot = alt.Chart(top_bottom_states).mark_point(size=100, color='blue').encode(
    x=alt.X('US State:N', title='US State', sort='-y'),  # X-axis: US State (sorted by average hours)
    y=alt.Y('Hours_Spent_Volunteering:Q', title='Average Hours Spent Volunteering')  # Y-axis: Hours Volunteered
)

# Step 4: Combine scatter plot and labels
chart = scatter_plot + top_bottom_labels

# Step 5: Show the chart with title and adjusted axes
chart = chart.properties(
    title='Top 5 and Bottom 5 States by Average Hours Spent Volunteering',
    width=400,   # Set a custom width for the chart
    height=00   # Set a custom height for the chart
).configure_axisY(
    labelAngle=0  # Make sure the Y-axis labels (hours) are readable
).configure_axisX(
    labelAngle=45  # Rotate X-axis labels to avoid overlap
)
chart.show()
```
...
```{python}
filtered_data = volunteering_frequencies[
    (volunteering_frequencies['US State'].notna()) & 
    (volunteering_frequencies['Voted_In_Local_Election'].isin(['Yes', 'No']))
]
state_counts = filtered_data.groupby(['US State', 'Voted_In_Local_Election']).size().reset_index(name='Count')

wide_format = state_counts.pivot(index='US State', columns='Voted_In_Local_Election', values='Count').reset_index()

wide_format.columns.name = None 
wide_format.rename(columns={'No': 'Voted_No', 'Yes': 'Voted_Yes'}, inplace=True)

wide_format['Total_Votes'] = wide_format['Voted_No'] + wide_format['Voted_Yes']

print(wide_format.head(10))
```
...
filtered_data = volunteering_frequencies[
    (volunteering_frequencies['Boycott_Based_On_Values'] != "Not in Universe") &
    (volunteering_frequencies['Boycott_Based_On_Values'].notna())
]
...
chart = alt.Chart(combined_counts).mark_bar().encode(
    y=alt.Y('Volunteering_Frequency:O', title='Volunteering Frequency'),  
    x=alt.X('Frequency:Q', title='Count'),  
    color=alt.Color('Boycott_Based_On_Values:N', title='Boycotted', scale=alt.Scale(domain=['Yes', 'No'], range=['blue', 'purple']), legend=alt.Legend(title="Voted")),  
...
filtered_data = volunteering_frequencies[
    (volunteering_frequencies['Contacted_Public_Official'] != "Not in Universe") &
    (volunteering_frequencies['Contacted_Public_Official'].notna())
]
...
chart = alt.Chart(combined_counts).mark_bar().encode(
    y=alt.Y('Volunteering_Frequency:O', title='Volunteering Frequency'),  
    x=alt.X('Frequency:Q', title='Count'),  
    color=alt.Color('Contacted_Public_Official:N', title='Contacted Elected Official', scale=alt.Scale(domain=['Yes', 'No'], range=['blue', 'purple']), legend=alt.Legend(title="Voted")),  
