...
# If they voted or not in the local election
```{python}
...
# Assuming cev_all_2021_filter is your DataFrame
# Filter rows where 'voted_local_election' is 'Yes' or 'No'
...
chart = alt.Chart(filtered_data).mark_boxplot().encode(
    x='Voted_In_Local_Election:N',   # Categorical axis (Yes or No)
    y='Age:Q',                    # Quantitative axis (age)
    color='Voted_In_Local_Election:N' # Different color for each group (Yes or No)
).properties(
...
frequency_counts = filtered_data['Discussed_Issues_With_Friends_Family'].value_counts()
