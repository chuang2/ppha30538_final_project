...
```{python}
#note: this code chunk may take a couple minutes to run
...
Link: https://electionstudies.org/data-center/2020-time-series-study/
...
polarization = pd.read_csv(
    "/Users/charleshuang/Documents/GitHub/student30538/problem_sets/final_project/data/anes_timeseries_2020_csv_20220210.csv")
```
...
Attribution: The use of the crosswalk + merge strategy comes from Pset6.
...
The first we will call Share of Outliers. This one 
is used for data exploration here, we are not 
directly graphing the relationship between 
volunteering and polarization with it.
The objective here is to 
capture the number of people within the two
 most-popular American
political parties (the Democratic and Republican 
parties) that 
diverge from the prevailing political alignment 
in their 
respective parties. That means the number of Democrats that 
identify as Conservative, and the number of
Republicans that identify as Liberal. In the book "Political
Polarizationin American Politics," edited by Daniel J. Hopkins,
there is a section in the first chapter 
entitled "Americans are not polarized,
just better sorted." This section notes that "Self-identified 
Democrats have become
more homogeneously liberal and 
self-identified Republicans more
homogeneously conservative." And though the title 
of the section might suggest otherwise, the authors 
of this section make sure to note that: "...none of this is to 
argue that sorting is unimportant.
It is, and sorting contributes to 
the extreme partisanship we see in 
Washington and many of our state 
capitals." So we would argue that while this 
might not strictly measure polarization, it is a 
useful enough dynamic to inform our understanding 
of it.


First, make function that subsets by state, and then groups
...
Attribution: Asked ChatGPT for help troubleshooting the for loops, the main fix was syntactic, mainly the indexing
in the if and elif statements. 

Then make bigger function to hold everything together
```{python}
...
Then make bigger function to produce results by each state by subsetting for a given state
```{python}
...
Save to a dataframe and produce graphs of both the count of outliers by state, and the percentage of outliers
as a share of total population that identifies as Republican or Democrat.
```{python}
...
percent_graph_outliers.show()
percent_graph_outliers.save("./data/percent_graph_outliers.png")
...
sum_graph_outliers.show()
sum_graph_outliers.save("./data/sum_graph_outliers.png")

```

Now, on to the second measure of polarization.
...
"Polarization in the sense of spread can be measured as the value of the agent with the highest belief value 
minus the value of the agent with the lowest belief value (sometimes called the ‘range’ of the data)."
...
Attribution: I utilized this article 
https://www.w3resource.com/pandas/dataframe/dataframe-pivot.php
to better understand the use of pivot, and then 
Asked ChatGPT how to flatten a multi-index column when 
my use of .pivot() produced a multi-dimensional column.
Its answer involved the use of the list comprehension, 
and I then renamed the resulting columns.
...
Attribution: I asked ChatGPT how to modify the color gradiant,
after also 
reviewing Altair documentation on color usage. 
...
Dem_Graph_Spread_1.save("./data/Dem_Graph_Spread_1.png")

Dem_Graph_Spread_2.show()
Dem_Graph_Spread_2.save("./data/Dem_Graph_Spread_2.png")
```
...
Repub_Graph_Spread_1.save("./data/Repub_Graph_Spread_1.png")

Repub_Graph_Spread_2.show()
Repub_Graph_Spread_2.save("./data/Repub_Graph_Spread_2.png")
```
...
Ideological_Position_US.save("./data/Ideological_Position_US.png")
```
...
#Attribution: Asked ChatGPT how to add a regression line with slope calculation to the chart. ChatGPT suggested using linregress from scipy.stats package and provided assistance with the syntax
...
slope, intercept, r_value, p_value, std_err = linregress(
    main_group["Hours_Spent_Volunteering"],
    main_group["political_engagement_score"]
)
...
Regression_Graph = alt.Chart(main_group).transform_regression(
    "Hours_Spent_Volunteering", "political_engagement_score", method="linear"
).mark_line(color="red").encode(
    alt.X("Hours_Spent_Volunteering", title="Average Volunteer Hours"),
    alt.Y("political_engagement_score", title="Average Political Engagement Score")
)

# Create the scatter plot
Correlation_Graph = alt.Chart(main_group).mark_circle(size=100).encode(
    alt.X("Hours_Spent_Volunteering", title="Average Volunteer Hours"),
    alt.Y("political_engagement_score", title="Average Political Engagement Score"),
    alt.Color("US State 2", title="State"),
    tooltip=["US State 2", "Hours_Spent_Volunteering", "political_engagement_score"]
)

# Add slope annotation
Slope_Annotation = alt.Chart(pd.DataFrame({"x": [main_group["Hours_Spent_Volunteering"].max()], "y": [main_group["political_engagement_score"].max()]})).mark_text(
    align="left", dx=-10, dy=10, color="black"
).encode(
    x="x:Q",
    y="y:Q",
    text=alt.value(slope_text)  # Display the slope as text
)

# Combine the scatter plot, regression line, and annotation
Correlation_Graph_With_Regression = alt.layer(
    Correlation_Graph,  # Scatter plot
    Regression_Graph,   # Regression line
    Slope_Annotation    # Slope annotation
).properties(
    title="Correlation between Volunteer Hours and Political Engagement with Regression and Slope",
    width=600,
    height=400
)

# Show the combined chart
Correlation_Graph_With_Regression.show()


```
Correlation between volunteer hours and political polarization


```{python}

def plot_volunteerism_vs_polarization(df, volunteer_metric, polarization_metric, party="Dem"):
    """
    Plots the relationship between volunteerism and political polarization.
    """
    polarization_plot = alt.Chart(df).mark_circle(size=100).encode(
        alt.X(volunteer_metric, title="Average Volunteer Hours"),
        alt.Y(
            polarization_metric,
            title=f"Spread of Views on {
                party} Party" if party else "Spread of Views",
            # Optional: Avoid forcing the scale to start at 0
            scale=alt.Scale(zero=False)
        ),
        alt.Color("US State:N", title="State"),
        # Add tooltips for interactivity
        tooltip=["US State", volunteer_metric, polarization_metric]
    ).properties(
        title=f"Volunteerism vs. Political Polarization ({party} Party)",
        width=600,
        height=400
    )

    return polarization_plot


d_polar_plot = plot_volunteerism_vs_polarization(
    df=main_group,
    volunteer_metric="Hours_Spent_Volunteering",
    polarization_metric="Spread_Dem",
    party="Dem"
)

d_polar_plot.show()

r_polar_plot = plot_volunteerism_vs_polarization(
    df=main_group,
    volunteer_metric="Hours_Spent_Volunteering",
    polarization_metric="Spread_Repub",
    party="Repub"
)
